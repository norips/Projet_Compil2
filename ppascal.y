...
D_entp: Dep NPro '(' L_argt ')'             {$$ = $2; if(pro(&h_table,$2,type(typeVoid),$4) == NULL){fprintf(stderr, KRED "Near line : %d\tAlready defined %s\n" KNRM,line,$2);exit(-1);} }

D_entf: Def NFon '(' L_argt ')' ':' TP      {$$ = $2; if(fun(&h_table,$2,$7,$4)== NULL) {fprintf(stderr, KRED "Near line : %d\tAlready defined %s\n" KNRM,line,$2);exit(-1);} }

D: D_entp L_vart C                          {symbolTag *f = getID(&h_table,$1); if(f->type != typePro) {yyerror("Identifier already declared");} f->_fun.local = $2; f->_fun.corps = $3;}
